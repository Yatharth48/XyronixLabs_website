...
export default function AboutUsPage() {
  ...
  return (
    <div className={`min-h-screen ${darkMode ? 'dark bg-gray-950 text-gray-100' : 'bg-gray-50 text-gray-900'} transition-colors duration-300`}>
      ...
      {/* Navigation */}
      <nav className={`fixed top-0 left-0 right-0 z-40 ${darkMode ? 'bg-gray-900/80' : 'bg-white/80'} backdrop-blur-md transition-colors duration-300`}>
        <div className="container mx-auto px-4 py-4 flex justify-between items-center">
          ...
          >
            <Rocket className={`h-8 w-8 ${darkMode ? 'text-purple-400' : 'text-purple-600'}`} />
          ...
          <div className="flex items-center space-x-6">
            ...
            >
              {['about', 'mission', 'research', 'team', 'founders', 'values'].map((section, index) => (
                <button 
                  key={section}
            ...
            <motion.button
              ...
              onClick={toggleTheme}
              className={`p-2 rounded-full ${darkMode ? 'bg-gray-800 text-yellow-400' : 'bg-gray-200 text-gray-700'} hover:bg-opacity-80 transition-colors`}
      ...
      <section id="about" ref={sectionRefs.about} className="relative overflow-hidden pt-24 min-h-screen flex items-center">
        <div className={`absolute inset-0 ${darkMode ? 'bg-gradient-to-b from-purple-900/20 to-gray-950' : 'bg-gradient-to-b from-purple-100 to-white'} z-0`} />
        ...
        <div className="container mx-auto px-4 relative z-10">
          ...
          >
            ...
            >
              ...
              <p className={`text-xl md:text-2xl ${darkMode ? 'text-gray-300' : 'text-gray-700'} mb-8`}>
            ...
            >
              <motion.button
                ...
                whileTap={{ scale: 0.95 }}
                className={`px-6 py-3 ${darkMode ? 'bg-purple-600 hover:bg-purple-700' : 'bg-purple-500 hover:bg-purple-600'} rounded-full font-medium transition-colors text-white flex items-center space-x-2 group`}
      ...
      {/* Stats Section */}
      <section ref={statsRef} className={`py-20 ${darkMode ? 'bg-gray-900' : 'bg-white'} transition-colors duration-300 relative overflow-hidden`}>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
              {stats.map((stat, index) => (
                <Tilt
                  ...
                  className={`p-6 rounded-xl ${darkMode ? 'bg-gray-800/80 backdrop-blur-sm' : 'bg-gray-100/80 backdrop-blur-sm'} transition-all duration-300 border ${darkMode ? 'border-purple-900/30' : 'border-purple-200/30'}`}
                >
                  <div className="mb-3">
                    {stat.icon}
                  </div>
                  <div className={`text-4xl md:text-5xl font-bold mb-2 ${darkMode ? 'text-purple-400' : 'text-purple-600'}`}>
                    {statsInView && (
                      <CountUp 
                        end={stat.value} 
                        duration={2.5} 
                  ...
                  <p className={`text-sm md:text-base ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>{stat.label}</p>
                </Tilt>
              ))}
            </div>
          </motion.div>
      ...
      {/* Company Overview */}
      <section className={`py-20 ${darkMode ? 'bg-gray-950' : 'bg-gray-50'} transition-colors duration-300 relative overflow-hidden`}>
        ...
        <div className="container mx-auto px-4 relative z-10">
          ...
          >
            ...
            <p className={`text-lg ${darkMode ? 'text-gray-300' : 'text-gray-700'} mb-6`}>
              Xyronix Labs is dedicated to innovation in AI, IoT, robotics, and data science. 
              We aim to bridge the gap between cutting-edge technology and real-world applications, 
              creating solutions that enhance lives and businesses.
            </p>
            <p className={`text-lg ${darkMode ? 'text-gray-300' : 'text-gray-700'} mb-10`}>
              One of our flagship products, the Fire Early Warning and Detection System, ensures 
              safety with advanced AI and IoT capabilities, offering early fire detection, precise 
              localization, and efficient suppression.
            </p>
            ...
            <div className="grid md:grid-cols-2 gap-8">
              {companyFeatures.map((feature, index) => (
                <motion.div
                  ...
                  viewport={{ once: true }}
                  whileHover={{ y: -5 }}
                  className={`${darkMode ? 'bg-gray-800/80 hover:bg-gray-800/60' : 'bg-white/80 hover:bg-gray-100/80'} rounded-xl p-6 transition-all duration-300 shadow-lg backdrop-blur-sm border ${darkMode ? 'border-purple-900/20' : 'border-purple-200/20'}`}
                >
                  <div className={`${darkMode ? 'bg-purple-900/30' : 'bg-purple-100'} rounded-full p-3 w-fit mb-4`}>
                    {feature.icon}
                  </div>
      ...
      {/* Mission & Vision */}
      <section id="mission" ref={sectionRefs.mission} className={`py-20 ${darkMode ? 'bg-gray-900' : 'bg-white'} transition-colors duration-300 relative overflow-hidden`}>
        ...
        <div className="container mx-auto px-4 relative z-10">
          <div className="max-w-4xl mx-auto">
            ...
            >
              ...
              <div className={`h-1 w-20 ${darkMode ? 'bg-purple-500' : 'bg-purple-600'} mx-auto`}></div>
            </motion.div>
            ...
            <div className="grid md:grid-cols-2 gap-10">
              <Tilt options={{ max: 10, scale: 1.02, speed: 300 }}>
                <motion.div
                  ...
                  viewport={{ once: true }}
                  className={`${darkMode ? 'bg-gray-800/80' : 'bg-gray-100/80'} p-8 rounded-xl border ${darkMode ? 'border-gray-700/50' : 'border-gray-200/50'} shadow-lg transition-colors duration-300 backdrop-blur-sm`}
                >
                  <Target className={`h-12 w-12 ${darkMode ? 'text-purple-400' : 'text-purple-600'} mb-4`} />
                  <h3 className="text-2xl font-bold mb-4">Our Mission</h3>
                  <p className={`${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>
                    To create impactful technologies that enhance safety, efficiency, and quality of life 
                    through innovative AI and IoT solutions. We strive to make advanced technology accessible 
              ...
              <Tilt options={{ max: 10, scale: 1.02, speed: 300 }}>
                <motion.div
                  ...
                  className={`${darkMode ? 'bg-gray-800/80' : 'bg-gray-100/80'} p-8 rounded-xl border ${darkMode ? 'border-gray-700/50' : 'border-gray-200/50'} shadow-lg transition-colors duration-300 backdrop-blur-sm`}
                >
                  <Award className={`h-12 w-12 ${darkMode ? 'text-cyan-400' : 'text-cyan-600'} mb-4`} />
                  <h3 className="text-2xl font-bold mb-4">Our Vision</h3>
                  <p className={`${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>
                    Guided by the vision of a smarter and safer tomorrow, we focus on creating technologies 
                    that embody our motto, "Future is Here." We envision a world where intelligent systems 
                    seamlessly integrate into daily life, solving complex problems with elegant solutions.
                  </p>
        ...
        <div className="container mx-auto px-4 relative z-10">
          ...
          <div className="grid md:grid-cols-2 gap-12 items-center mb-20">
            ...
            >
              ...
              <p className={`${darkMode ? 'text-gray-300' : 'text-gray-700'} mb-6`}>
                Our research team develops advanced neural networks and machine learning algorithms that power our innovative solutions. 
                From computer vision systems that detect anomalies to predictive analytics that anticipate maintenance needs, 
                our AI technologies are at the forefront of the industry.
              </p>
              ...
              <div className="space-y-4">
                {aiCapabilities.map((capability, index) => (
                  <motion.div 
                    ...
                    viewport={{ once: true }}
                    className={`flex items-start space-x-3 p-3 rounded-lg ${darkMode ? 'bg-gray-800/50' : 'bg-white/50'} backdrop-blur-sm`}
                  >
                    <div className={`p-2 rounded-full ${darkMode ? 'bg-purple-900/30' : 'bg-purple-100'} mt-1`}>
                      {capability.icon}
                    </div>
                    <div>
                      <h4 className="font-semibold">{capability.title}</h4>
                      <p className={`text-sm ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>{capability.description}</p>
                    </div>
                  </motion.div>
                ))}
              </div>
          ...
          <div className="grid md:grid-cols-2 gap-12 items-center">
            ...
            <motion.div
              ...
              viewport={{ once: true }}
            >
              <h3 className="text-2xl font-bold mb-4">IoT & Robotics Integration</h3>
              <p className={`${darkMode ? 'text-gray-300' : 'text-gray-700'} mb-6`}>
                Our innovative approach combines IoT sensors with robotic systems to create intelligent, 
                autonomous solutions. These integrated systems can monitor environments, collect data, 
                and take action without human intervention, making them ideal for hazardous or remote locations.
              </p>
              ...
              <div className="space-y-4">
                {iotCapabilities.map((capability, index) => (
                  <motion.div 
                    ...
                    className={`flex items-start space-x-3 p-3 rounded-lg ${darkMode ? 'bg-gray-800/50' : 'bg-white/50'} backdrop-blur-sm`}
                  >
                    <div className={`p-2 rounded-full ${darkMode ? 'bg-cyan-900/30' : 'bg-cyan-100'} mt-1`}>
                      {capability.icon}
                    </div>
                    <div>
                      <h4 className="font-semibold">{capability.title}</h4>
          ...
          <motion.div
            ...
            className="mt-20"
          >
            <h3 className="text-2xl font-bold mb-8 text-center">Featured Research Projects</h3>
            
            <div className="grid md:grid-cols-3 gap-6">
              {researchProjects.map((project, index) => (
                <Tilt key={index} options={{ max: 15, scale: 1.03, speed: 400 }}>
                  <motion.div
                    initial={{ opacity: 0, y: 20 }}
                    whileInView={{ opacity: 1, y: 0 }}
                    transition={{ duration: 0.5, delay: index * 0.1 }}
                    viewport={{ once: true }}
                    whileHover={{ y: -10 }}
                    className={`rounded-xl overflow-hidden shadow-lg ${darkMode ? 'bg-gray-800/70' : 'bg-white/70'} backdrop-blur-sm border ${darkMode ? 'border-gray-700/50' : 'border-gray-200/50'}`}
                  >
                    <div className="h-48 relative overflow-hidden">
                      ...
                      />
                      <div className={`absolute top-3 right-3 px-2 py-1 rounded-full text-xs font-medium ${darkMode ? 'bg-purple-600/90' : 'bg-purple-500/90'} text-white`}>
                        {project.category}
                      </div>
                    </div>
                    <div className="p-5">
                      <h4 className="text-lg font-bold mb-2">{project.title}</h4>
                      <p className={`text-sm ${darkMode ? 'text-gray-300' : 'text-gray-600'} mb-4`}>{project.description}</p>
                      <div className="flex flex-wrap gap-2">
                        {project.technologies.map((tech, i) => (
                          <span key={i} className={`text-xs px-2 py-1 rounded-full ${darkMode ? 'bg-gray-700 text-gray-300' : 'bg-gray-200 text-gray-700'}`}>
                            {tech}
                          </span>
                        ))}
                      </div>
      ...
      <section id="founders" ref={sectionRefs.founders} className={`py-20 ${darkMode ? 'bg-gray-950' : 'bg-gray-50'} transition-colors duration-300 relative overflow-hidden p-16 mb-6`}>
        ...
        <div className="container mx-auto px-12 relative z-10 text-justify">
          ...
          >
            ...
            <p className={`text-xl ${darkMode ? 'text-gray-300' : 'text-gray-700'} max-w-3xl mx-auto`}>
              The visionaries who established Xyronix Labs and continue to lead our innovation.
            </p>
            <div className={`h-1 w-20 ${darkMode ? 'bg-purple-500' : 'bg-purple-600'} mx-auto mt-6`}></div>
          </motion.div>
            ...
            {founders.map((founder, index) => (
              ...
              >
                ...
                <div className={`${index % 2 === 1 ? "md:order-1" : ""}`}>
                  ...
                  <p className={`mt-2 text-xl font-medium ${darkMode ? 'text-purple-400' : 'text-purple-600'}`}>{founder.role}</p>
                  ...
                  <div className="mt-8 flex space-x-4">
                    {founder.social.linkedin && (
                      <motion.a
                        ...
                        className={`rounded-full ${darkMode ? 'bg-gray-800 text-gray-300 hover:bg-purple-600 hover:text-white' : 'bg-gray-200 text-gray-700 hover:bg-purple-500 hover:text-white'} p-3 transition-colors`}
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
            {coreValues.map((value, index) => (
              <motion.div
                ...
                className={`${darkMode ? 'bg-gray-800/80 hover:bg-gray-800/60' : 'bg-white/80 hover:bg-gray-100/80'} p-6 rounded-xl transition-all shadow-lg border ${darkMode ? 'border-gray-700/50' : 'border-gray-200/50'} backdrop-blur-sm`}
              >
                <div className={`${darkMode ? 'bg-purple-900/30' : 'bg-purple-100'} rounded-full p-3 w-fit mb-4`}>
                  {value.icon}
                </div>
            ...
            {timeline.map((item, index) => (
              ...
              >
                ...
                {index < timeline.length - 1 && (
                  <div className={`absolute left-4 md:left-1/2 top-16 bottom-0 w-0.5 ${darkMode ? 'bg-gray-700' : 'bg-gray-300'} md:-translate-x-1/2`}></div>
                )}
                
                {/* Timeline dot */}
                <div className={`absolute left-4 md:left-1/2 top-6 w-8 h-8 rounded-full ${darkMode ? 'bg-purple-600' : 'bg-purple-500'} flex items-center justify-center md:-translate-x-1/2 z-10`}>
                  <div className={`w-4 h-4 rounded-full ${darkMode ? 'bg-gray-900' : 'bg-white'}`}></div>
                </div>
                
                <div className={`md:w-1/2 ${index % 2 === 0 ? 'md:pr-12 md:text-right' : 'md:pl-12 md:ml-auto'} pl-16 md:pl-0`}>
                  <div className={`p-6 rounded-xl shadow-lg ${darkMode ? 'bg-gray-800/80' : 'bg-gray-100/80'} backdrop-blur-sm`}>
                    <div className={`text-sm font-semibold ${darkMode ? 'text-purple-400' : 'text-purple-600'} mb-2`}>{item.year}</div>
        ...
        {selectedMember && (
          ...
          >
            <motion.div
              ...
              animate={{ scale: 1, y: 0 }}
              exit={{ scale: 0.9, y: 20 }}
              className={`relative max-w-3xl w-full ${darkMode ? 'bg-gray-900' : 'bg-white'} rounded-2xl shadow-2xl overflow-hidden`}
            ...
            >
              ...
              <div className="grid md:grid-cols-2">
                <div className="aspect-square">
                  <Image
                    src={selectedMember.image || "/placeholder.svg?height=600&width=600"}
                  ...
                  <div className={`flex-grow overflow-y-auto ${darkMode ? 'text-gray-300' : 'text-gray-700'} space-y-4 pr-2`}>
                    ...
                    <div className={`mt-6 p-4 rounded-lg ${darkMode ? 'bg-gray-800' : 'bg-gray-100'}`}>
                      ...
                      <div className="flex flex-wrap gap-2">
                        ...
                        )) || ['AI', 'IoT', 'Robotics', 'Data Science', 'Cloud Computing'].map((skill, i) => (
                          <span 
                            key={i} 
                            className={`px-3 py-1 rounded-full text-sm ${darkMode ? 'bg-gray-700 text-gray-300' : 'bg-gray-200 text-gray-800'}`}
                          >
                  ...
                  <div className="mt-6 flex space-x-3">
                    {selectedMember.social.linkedin && (
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className={`rounded-full ${darkMode ? 'bg-gray-800 text-gray-300 hover:bg-purple-600 hover:text-white' : 'bg-gray-200 text-gray-700 hover:bg-purple-500 hover:text-white'} p-2 transition-colors`}
...
function TeamMemberCard({ member, index, isFounder, darkMode, onClick }: { 
  ...
  return (
    <motion.div
      ...
      transition={{ duration: 0.5, delay: index * 0.1 }}
      viewport={{ once: true }}
      className={`relative overflow-hidden rounded-xl ${darkMode ? 'bg-gray-800/80' : 'bg-white/80'} group shadow-lg cursor-pointer backdrop-blur-sm border ${darkMode ? 'border-gray-700/50' : 'border-gray-200/50'}`}
    ...
    >
      ...
      <div className="p-6">
        ...
        <p className={`${darkMode ? 'text-purple-400' : 'text-purple-600'} mt-1`}>{member.role}</p>
      </div>
    ...
    <motion.div className="relative w-full h-full">
      ...
      >
        ...
        <motion.div
          ...
          animate={{ opacity: [0.5, 0.8, 0.5], y: [20, 30, 20], scale: [0.8, 1, 0.8] }}
...
const coreValues = [
  ...
  {
    title: "Integrity",
    description: "We operate with honesty, transparency, and ethical standards in all our endeavors.",
    icon: <Shield className="h-6 w-6 text-purple-400" />,
  },
  {
    title: "Impact",
    description: "We measure our success by the positive difference our technology makes in the world.",
...
const companyFeatures = [
  {
    ...
    description: "Our scalable and reliable solutions cater to diverse industries, including residential, commercial, and industrial sectors.",
    icon: <Building2 className="h-10 w-10 text-purple-400" />,
  },
  {
    title: "Intelligent Systems",
    description: "By leveraging intelligent systems powered by neural networks and robust cloud-based platforms, we drive efficiency, safety, and sustainable growth.",
    icon: <Brain className="h-10 w-10 text-cyan-400" />,
  },
  {
    title: "Rapid Innovation",
    description: "Our agile development approach allows us to quickly adapt to changing needs and emerging technologies.",
    icon: <Zap className="h-10 w-10 text-yellow-400" />,
  },
  {
    title: "Secure Infrastructure",
    description: "We prioritize security in all our solutions, ensuring data protection and system integrity.",
...
const stats = [
  ...
  { 
    ...
    suffix: "%",
    icon: <div className="mx-auto w-12 h-12 flex items-center justify-center rounded-full bg-green-900/30">
...
const aiCapabilities = [
  {
    ...
    description: "Advanced object detection and recognition for real-time monitoring and analysis",
    icon: <Eye className="h-4 w-4 text-purple-400" />
  },
  {
    title: "Predictive Analytics",
    description: "Forecasting potential issues before they occur using historical and real-time data",
    icon: <BarChart3 className="h-4 w-4 text-purple-400" />
  },
  {
    title: "Natural Language Processing",
    description: "Understanding and processing human language for intuitive user interfaces",
    icon: <MessageSquare className="h-4 w-4 text-purple-400" />
  },
  {
    title: "Deep Learning",
    description: "Multi-layered neural networks that continuously improve with more data",
...
const iotCapabilities = [
  {
    ...
    description: "Distributed networks of sensors for comprehensive environmental monitoring",
    icon: <Wifi className="h-4 w-4 text-cyan-400" />
  },
  {
    title: "Edge Computing",
    description: "Processing data at the source to reduce latency and bandwidth usage",
    icon: <Cpu className="h-4 w-4 text-cyan-400" />
  },
  {
    title: "Autonomous Systems",
    description: "Self-operating robotic systems that can navigate and perform tasks independently",
    icon: <Cog className="h-4 w-4 text-cyan-400" />
  },
  {
    title: "Real-time Monitoring",
    description: "Continuous data collection and analysis for immediate response capabilities",
...
const researchProjects = [
  {
    ...
    description: "Advanced neural network for early fire detection with 99.8% accuracy and minimal false positives",
    image: "/placeholder.svg?height=300&width=400",
    category: "AI",
    technologies: ["TensorFlow", "Computer Vision", "Thermal Imaging", "Edge AI"]
  },
  {
    title: "Autonomous Response Robots",
    description: "Self-navigating robots that can locate and suppress fires in complex environments",
    image: "/placeholder.svg?height=300&width=400",
    category: "Robotics",
  ...
  },
  {
    ...
    description: "Low-power, high-reliability sensor network with mesh topology for comprehensive coverage",
    image: "/placeholder.svg?height=300&width=400",
    category: "IoT",
...
import { Users, CheckCircle, Globe, ThumbsUp, Eye, MessageSquare, Activity } from 'lucide-react'
